...
def datosbarbero(request):
    return render(request, 'Gotti/VistaBarbero/datosbarbero.html')
...
def ver_todos_horarios(request, idServicio):
    ...
    horarios = BloqueHorario.objects.filter(servicio=servicio)  # Filtra los horarios para el servicio actual
    return render(request, 'Gotti/VistaBarbero/ver_todos_horarios.html', {'servicio': servicio, 'horarios': horarios})
...
def horarios_disponibles_cliente(request, idServicio):
    ...
    # Filtrar los bloques de horario disponibles a partir de hoy
    hoy = timezone.now().date()
    bloques_disponibles = BloqueHorario.objects.filter(servicio=servicio, fecha__gte=hoy)
